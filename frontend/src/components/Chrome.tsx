...
export function TopBar() {
  ...
  return (
    <header className="sticky top-0 z-20 flex items-center justify-between border-b border-line bg-bg px-4 py-3 lg:px-8">
      <div className="flex items-center gap-2 lg:hidden">
        <Owl className="h-8 w-8" />
        <span className="font-display text-xl font-bold text-feather">lingo</span>
      </div>
      <div className="ml-auto flex items-center gap-3 text-sm font-extrabold">
        <ThemeToggle compact />
...
export function SideNav() {
  ...
  return (
    <aside className="hidden w-[224px] shrink-0 border-r border-line bg-bg lg:flex lg:flex-col lg:py-6">
          ...
          return (
            <Link
              ...
              className={`flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-extrabold uppercase tracking-wide ${
                active ? "border-2 border-macaw bg-select text-macaw" : "text-muted hover:bg-hover"
              }`}
            >
              <span>{l.icon}</span>
              {l.label}
...
export function BottomNav() {
  const path = usePathname();
  return (
    <nav className="fixed inset-x-0 bottom-0 z-20 flex border-t border-line bg-bg lg:hidden">
      {links.map((l) => {
        const active = path === l.href;
        return (
          <Link
  ...
  return (
    <aside className="hidden w-[300px] shrink-0 p-4 xl:block">
      <div className="rounded-2xl border-2 border-line bg-surface p-4">
        <p className="text-xs font-extrabold uppercase text-muted">Daily goal</p>
        <p className="mt-1 font-display text-lg font-bold text-fg">Earn {goal} XP today</p>
      ...
      <div className="mt-4 rounded-2xl border-2 border-line bg-surface p-4">
        <p className="text-xs font-extrabold uppercase text-muted">Spanish</p>
        <p className="mt-1 text-sm font-bold text-fg">English → Spanish path. Keep the streak alive.</p>
      </div>
    </aside>
  );
}
