...
function SkillBubble({ skill, index }: { skill: SkillNode; index: number }) {
  ...
  const color = done ? "bg-bee shadow-[0_8px_0_0_#E5A800]" : locked ? "bg-line shadow-[0_8px_0_0_#2A3A42]" : "bg-feather shadow-[0_8px_0_0_#46A302]";
  const ring = `conic-gradient(#58CC02 ${skill.progress * 360}deg, var(--color-line) 0deg)`;
  const inner = (
    <div className="flex flex-col items-center" style={{ transform: `translateX(${offsets[index % offsets.length]}px)` }}>
      <div className="relative h-[88px] w-[88px] rounded-full p-[6px]" style={{ background: locked ? "var(--color-line)" : ring }}>
        <div
      ...
      <p className="mt-2 max-w-[140px] text-center text-sm font-extrabold text-fg">{skill.title}</p>
...
export function LearningPath() {
  ...
  if (isLoading) return <p className="p-8 font-bold text-muted">Loading your path…</p>;
