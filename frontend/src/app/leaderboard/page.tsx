    ...
    <div className="flex min-h-screen">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <main className="mx-auto w-full max-w-xl px-4 py-6 pb-24">
          ...
          <p className="mb-4 text-sm font-bold text-wolf">All-time XP among seeded learners</p>
          {isLoading && <p>Loading…</p>}
          <ol className="divide-y divide-swan rounded-2xl border-2 border-swan">
            {data?.map((row) => (
              <li
                key={row.user_id}
                className={`flex items-center justify-between px-4 py-3 ${row.is_you ? "bg-[#ddf4ff]" : ""}`}
              >
